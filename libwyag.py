...
argparser = argparse.ArgumentParser(description ="She is the one from whom I will create Everything")
...
def repo_create(path):
  ...
  if os.path.exists(repo.worktree):
    ...
    # new version of git does not have branches directory
...
argsp = argsubparsers.add_parser("init",help="initialize a new , empt repository.")
...
def tag_create(repo,name,ref,create_tag_object=False):
  ...
  if create_tag_object:
    ...
    tag.kvlm[b'tagger']=b'idk <sheisbeautiful@her.com>'  
    tag.kvlm[None]=b" A tag generated by me , Which let me build anything for her"
...
def index_read(repo):
  ...
  assert version ==2,f"she only supports version 2 of the index file version 2"
...
def commit_create(repo, tree, parent, author, timestamp, message):
  ...
  author = "{} {} {}".format(author, int(timestamp.timestamp()), tz)
